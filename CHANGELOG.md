...
- Auto-set profile name on first boot, configurable via `SIGNAL_PROFILE_NAME` (empty disables). Marker file at `~/.claude/channels/signal/.profile-set` tracks last-applied value so the bridge only re-applies on change.
