...
To opt out of history persistence entirely, set `SIGNAL_DISABLE_HISTORY=true` in `.env`. The bridge then skips `messages.db` writes and reads; `chat_messages`, `react`, and `mark_read` throw a clear error when called. Live channel routing and the `authors.json` display-name cache continue to work. This eliminates plain-text-at-rest persistence of message bodies but does not affect Signal's transport-layer E2E semantics — signal-cli still handles message decryption in its own data dir at `~/.local/share/signal-cli/`, which contains the account keys and may cache recent messages independently. For a fully ephemeral deploy, also point `SIGNAL_CLI_CONFIG` at a tmpfs or container-bound directory and accept that signal-cli's identity keys won't survive restart.
